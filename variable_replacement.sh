...
do
  ...
  envsubst '$AM_URL $AM_ADMIN_URL $ANALYTICS_API_URL $ENABLE_ANALYTICS_DASH $ENABLE_ANALYTICS_CUSTOM_REPORTING $ENABLE_ANALYTICS_REPORTING $ENABLE_APPLICATION_POLICY $ENABLE_EXTENDED_SAML_CONFIG $ENABLE_OIDC_JOURNEYS $ENABLE_SAML_JOURNEYS $ENABLE_GOVERNANCE $ENABLE_GOVERNANCE_DEV $ENABLE_CERTS_UI $ENABLE_WORKFORCE $ENABLE_WORKFORCE_ADVANCED_SYNC $ENABLE_WSFED $ENABLE_TEMPLATE_BUILDER $IDM_REST_URL $IDM_ADMIN_URL $IDM_UPLOAD_URL $IDM_EXPORT_URL $ENDUSER_UI_URL $ENDUSER_CLIENT_ID $ADMIN_CLIENT_ID $PLATFORM_UI_LOCALE $PLATFORM_ADMIN_URL $PLATFORM_UI_IS_FRAAS $RCS_AGENT_ENABLED $FRAAS_LOGGING_URL $FRAAS_DEFAULT_REALM $FRAAS_ADMIN_MANAGED_OBJECT_NAME $FRAAS_ENV_URL $SHOW_ESV_UI $MENUS_FILE $ROUTES_FILE $FRAAS_MONITORING_URL $GOOGLE_FONTS_API_KEY $GOOGLE_MAPS_API_KEY $ENABLE_AUTO_ACCESS $AUTO_ACCESS_API_URL $AUTO_ACCESS_JAS_URL $AUTO_ACCESS_TENANT_ID $ALIGN_GOTO_PRECEDENCE $FRAAS_PROMOTION_URL $FRAAS_FEDERATION_ENFORCEMENT_URL $LOWER_CONFIG_INGRESS $UPPER_CONFIG_EGRESS $IGA_API_URL $AUTO_ACCESS_REPORTS_URL $IGA_ORCHESTRATION_API_URL $PINGFEDERATE_URL $PROMOTE_APPS_VIA_API $ENABLE_NEW_MULTISELECT $SCRIPT_BINDINGS_API_ENABLED $NODE_DESIGNER_ENABLED' < "/tmp/$(basename $file).tmpl" > "$file"
done
