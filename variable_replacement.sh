...
do
  ...
  envsubst '$ALLOWABLE_INGRESS_URLS $AM_URL $AM_ADMIN_URL $ANALYTICS_API_URL $ENABLE_ANALYTICS_DASH $ENABLE_ANALYTICS_CUSTOM_REPORTING $ENABLE_ANALYTICS_REPORTING $ENABLE_APPLICATION_POLICY $ENABLE_JOURNEY_AI $ENABLE_OVERRIDE_SESSION_TIMEOUTS $ENABLE_GOVERNANCE $ENABLE_GOVERNANCE_DEV $ENABLE_MTLS $ENABLE_SAML_SIGNING_CHECKBOX $ENABLE_TABLE_COLUMN_RESIZING $ENABLE_WORKFORCE $ENABLE_WSFED $ENABLE_TEMPLATE_BUILDER $IDM_REST_URL $IDM_ADMIN_URL $IDM_UPLOAD_URL $IDM_EXPORT_URL $ENDUSER_UI_URL $ENDUSER_CLIENT_ID $ADMIN_CLIENT_ID $PLATFORM_UI_LOCALE $PLATFORM_ADMIN_URL $PLATFORM_UI_IS_FRAAS $RCS_AGENT_ENABLED $FRAAS_LOGGING_URL $FRAAS_DEFAULT_REALM $FRAAS_ADMIN_MANAGED_OBJECT_NAME $FRAAS_ENV_URL $SHOW_ESV_UI $MENUS_FILE $ROUTES_FILE $FRAAS_MONITORING_URL $GOOGLE_FONTS_API_KEY $GOOGLE_MAPS_API_KEY $MTLS_FQDN $SALESFORCE_ID $ENABLE_AUTO_ACCESS $AUTO_ACCESS_API_URL $AUTO_ACCESS_JAS_URL $AUTO_ACCESS_TENANT_ID $ALIGN_GOTO_PRECEDENCE $FRAAS_PROMOTION_URL $FRAAS_FEDERATION_ENFORCEMENT_URL $LOWER_CONFIG_INGRESS $UPPER_CONFIG_EGRESS $IGA_API_URL $AUTO_ACCESS_REPORTS_URL $IGA_ORCHESTRATION_API_URL $PINGFEDERATE_URL $PROMOTE_APPS_VIA_API $ENABLE_NEW_MULTISELECT $ENABLE_PENDO $ENABLE_MANAGED_OBJECT_SCHEMA_EDITOR $ENABLE_MONITORING_UI $ENABLE_SDK_FEATURE $HELIX_ENVIRONMENT_ID $HELIX_ENVIRONMENT_URL $HELIX_JOURNEY_AGENT_VERSION' < "/tmp/$(basename $file).tmpl" > "$file"
done
