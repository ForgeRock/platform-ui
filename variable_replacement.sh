#!/bin/sh

# Copyright 2019-2026 ForgeRock AS. All Rights Reserved
#
# Use of this code requires a commercial software license with ForgeRock AS.
# or with one of its affiliates. All use shall be exclusively subject
# to such license between the licensee and ForgeRock AS.

echo "Replacing env vars in JS"
echo ""
echo "Setting AM URL as $AM_URL"
echo "Setting AM ADMIN URL as $AM_ADMIN_URL"
echo "Setting ANALYTICS API URL as $ANALYTICS_API_URL"
echo "Setting IDM REST URL as $IDM_REST_URL"
echo "Setting IDM ADMIN URL as $IDM_ADMIN_URL"
echo "Setting IDM UPLOAD URL as $IDM_UPLOAD_URL"
echo "Setting IDM EXPORT URL as $IDM_EXPORT_URL"
echo "Setting ENABLE AI AGENTS as $ENABLE_AI_AGENTS"
echo "Setting ENABLE AI DEVELOPMENT as $ENABLE_AI_DEV"
echo "Setting ENABLE ANALYTICS DASH as $ENABLE_ANALYTICS_DASH"
echo "Setting ENABLE ANALYTICS REPORT AI ASSISTANT as $ENABLE_ANALYTICS_REPORT_AI_ASSISTANT"
echo "Setting ANALYTICS DASHBOARD MODE as $ANALYTICS_DASHBOARD_MODE"
echo "Setting ENABLE AUTO CUSTOM REPORTS as $ENABLE_ANALYTICS_CUSTOM_REPORTING"
echo "Setting ENABLE AUTO REPORTS as $ENABLE_ANALYTICS_REPORTING"
echo "Setting ENABLE JOURNEY AI as $ENABLE_JOURNEY_AI"
echo "Setting ENABLE GOVERNANCE as $ENABLE_GOVERNANCE"
echo "Setting ENABLE GOVERNANCE DEVELOPMENT as $ENABLE_GOVERNANCE_DEV"
echo "Setting ENABLE GOVERNANCE AGENTS as $ENABLE_GOVERNANCE_AGENTS"
echo "Setting ENABLE GOVERNANCE ROLE MINING as $ENABLE_GOVERNANCE_ROLE_MINING"
echo "Setting ENABLE GOVERNANCE UNMANAGED APPS as $ENABLE_GOVERNANCE_UNMANAGED_APPS"
echo "Setting ENABLE WORKFORCE as $ENABLE_WORKFORCE"
echo "Setting ENABLE WSFED as $ENABLE_WSFED"
echo "Setting ENABLE TEMPLATE BUILDER as $ENABLE_TEMPLATE_BUILDER"
echo "Setting ENDUSER URL as $ENDUSER_UI_URL"
echo "Setting ENDUSER CLIENT ID as $ENDUSER_CLIENT_ID"
echo "Setting ADMIN CLIENT ID as $ADMIN_CLIENT_ID"
echo "Setting PLATFORM UI LOCALE as $PLATFORM_UI_LOCALE"
echo "Setting PLATFORM ADMIN URL as $PLATFORM_ADMIN_URL"
echo "Setting PLATFORM UI IS FRAAS as $PLATFORM_UI_IS_FRAAS"
echo "Setting RCS AGENT ENABLED as $RCS_AGENT_ENABLED"
echo "Setting FRAAS LOGGING URL as $FRAAS_LOGGING_URL"
echo "Setting FRAAS ENVIRONMENT URL as $FRAAS_ENV_URL"
echo "Setting FRAAS FEDERATION ENFORCEMENT URL as $FRAAS_FEDERATION_ENFORCEMENT_URL"
echo "Setting FRAAS MONITORING URL as $FRAAS_MONITORING_URL"
echo "Setting FRAAS PROMOTION URL as $FRAAS_PROMOTION_URL"
echo "Setting LOWER CONFIG INGRESS as $LOWER_CONFIG_INGRESS"
echo "Setting FRAAS_ALLOWABLE_CONFIG_PROMOTION_INGRESS_URLS as $ALLOWABLE_INGRESS_URLS"
echo "Setting UPPER CONFIG EGRESS as $UPPER_CONFIG_EGRESS"
echo "Setting FRAAS DEFAULT REALM as $FRAAS_DEFAULT_REALM"
echo "Setting FRAAS ADMIN NAME as $FRAAS_ADMIN_MANAGED_OBJECT_NAME"
echo "Setting MENUS FILE as $MENUS_FILE"
echo "Setting ROUTES FILE as $ROUTES_FILE"
echo "Setting GOOGLE FONTS API KEY as $GOOGLE_FONTS_API_KEY"
echo "Setting GOOGLE MAPS API KEY as $GOOGLE_MAPS_API_KEY"
echo "Setting AUTO ACCESS REPORTS URL as $AUTO_ACCESS_REPORTS_URL"
echo "Setting ALIGN GOTO PRECEDENCE as $ALIGN_GOTO_PRECEDENCE"
echo "Setting HELIX ENVIRONMENT ID as $HELIX_ENVIRONMENT_ID"
echo "Setting HELIX ENVIRONMENT URL as $HELIX_ENVIRONMENT_URL"
echo "Setting HELIX JOURNEY AGENT VERSION as $HELIX_JOURNEY_AGENT_VERSION"
echo "Setting HELIX JOURNEY BUILD AGENT VERSION as $HELIX_JOURNEY_BUILD_AGENT_VERSION"
echo "Setting IGA API URL as $IGA_API_URL"
echo "Setting IGA ORCHESTRATION API URL as $IGA_ORCHESTRATION_API_URL"
echo "Setting PING FEDERATE URL as $PINGFEDERATE_URL"
echo "Setting PROMOTE APPS VIA API as $PROMOTE_APPS_VIA_API"
echo "Setting ENABLE PROXY CONNECT as $ENABLE_PROXY_CONNECT"
echo "Setting ENABLE MTLS as $ENABLE_MTLS"
echo "Setting MTLS FQDN as $MTLS_FQDN"
echo "Setting SALESFORCE ID as $SALESFORCE_ID"
echo "Setting NODE VERSIONING ENABLED as $NODE_VERSIONING_ENABLED"
echo "Setting ENABLE SAML SIGNING CHECKBOX as $ENABLE_SAML_SIGNING_CHECKBOX"
echo "Setting ENABLE SAML SKIP METADATA SIGNATURE VERIFICATION as $ENABLE_SAML_SKIP_METADATA_SIGNATURE_VERIFICATION"
echo "Setting ENABLE MONITORING_UI as $ENABLE_MONITORING_UI"
echo "Setting ENABLE SDK FEATURE as $ENABLE_SDK_FEATURE"
echo "Setting ENABLE UI LOCALIZATION as $ENABLE_UI_LOCALIZATION"
echo "Setting DEPLOYMENT TYPE AS $DEPLOYMENT_TYPE"
echo "Setting ENABLE DEV APP TEMPLATES as $ENABLE_DEV_APP_TEMPLATES"
echo "Setting ENABLE SERVICES MIGRATION as $ENABLE_SERVICES_MIGRATION"
echo "Setting ENABLE PING ONE RECOGNIZE as $ENABLE_PING_ONE_RECOGNIZE"
echo "Setting PLATFORM UI AIR GAPPED as $PLATFORM_UI_AIR_GAPPED"
echo "Setting ENABLE SELF SERVICE as $ENABLE_SELF_SERVICE"
echo "Setting ENABLE ACCOUNTS as $ENABLE_ACCOUNTS"
echo "Setting AUTHORIZATION ENDPOINT as $AUTHORIZATION_ENDPOINT"
echo "Setting TOKEN ENDPOINT as $TOKEN_ENDPOINT"
echo "Setting REVOCATION ENDPOINT as $REVOCATION_ENDPOINT"
echo "Setting END SESSION ENDPOINT as $END_SESSION_ENDPOINT"

# Replace env vars in JavaScript files
for file in $@
do
  echo "Processing $file ..."

  # Use the existing JS file as template
  if [ ! -f "/tmp/$(basename $file).tmpl" ]; then
    cp "$file" "/tmp/$(basename $file).tmpl"
  fi

  envsubst '
    $ADMIN_CLIENT_ID
    $ALIGN_GOTO_PRECEDENCE
    $ALLOWABLE_INGRESS_URLS
    $AM_ADMIN_URL
    $AM_URL
    $ANALYTICS_API_URL
    $AUTO_ACCESS_REPORTS_URL
    $DEPLOYMENT_TYPE
    $ENABLE_ACCOUNTS
    $ENABLE_AI_AGENTS
    $ENABLE_AI_DEV
    $ENABLE_ANALYTICS_CUSTOM_REPORTING
    $ENABLE_ANALYTICS_DASH
    $ANALYTICS_DASHBOARD_MODE
    $ENABLE_ANALYTICS_REPORT_AI_ASSISTANT
    $ENABLE_ANALYTICS_REPORTING
    $ENABLE_DEV_APP_TEMPLATES
    $ENABLE_SERVICES_MIGRATION
    $ENABLE_GOVERNANCE
    $ENABLE_GOVERNANCE_DEV
    $ENABLE_GOVERNANCE_AGENTS
    $ENABLE_GOVERNANCE_ROLE_MINING
    $ENABLE_GOVERNANCE_UNMANAGED_APPS
    $ENABLE_JOURNEY_AI
    $ENABLE_MONITORING_UI
    $ENABLE_PING_ONE_RECOGNIZE
    $ENABLE_PROXY_CONNECT
    $ENABLE_MTLS
    $ENABLE_SAML_SIGNING_CHECKBOX
    $ENABLE_SAML_SKIP_METADATA_SIGNATURE_VERIFICATION
    $ENABLE_SDK_FEATURE
    $ENABLE_TEMPLATE_BUILDER
    $ENABLE_UI_LOCALIZATION
    $ENABLE_WORKFORCE
    $ENABLE_WSFED
    $ENDUSER_CLIENT_ID
    $ENDUSER_UI_URL
    $FRAAS_ADMIN_MANAGED_OBJECT_NAME
    $FRAAS_DEFAULT_REALM
    $FRAAS_ENV_URL
    $FRAAS_FEDERATION_ENFORCEMENT_URL
    $FRAAS_LOGGING_URL
    $FRAAS_MONITORING_URL
    $FRAAS_PROMOTION_URL
    $GOOGLE_FONTS_API_KEY
    $GOOGLE_MAPS_API_KEY
    $HELIX_ENVIRONMENT_ID
    $HELIX_ENVIRONMENT_URL
    $HELIX_JOURNEY_AGENT_VERSION
    $HELIX_JOURNEY_BUILD_AGENT_VERSION
    $IDM_ADMIN_URL
    $IDM_EXPORT_URL
    $IDM_REST_URL
    $IDM_UPLOAD_URL
    $IGA_API_URL
    $IGA_ORCHESTRATION_API_URL
    $LOWER_CONFIG_INGRESS
    $MENUS_FILE
    $MTLS_FQDN
    $NODE_VERSIONING_ENABLED
    $PINGFEDERATE_URL
    $PLATFORM_ADMIN_URL
    $PLATFORM_UI_AIR_GAPPED
    $PLATFORM_UI_IS_FRAAS
    $PLATFORM_UI_LOCALE
    $PROMOTE_APPS_VIA_API
    $RCS_AGENT_ENABLED
    $ROUTES_FILE
    $SALESFORCE_ID
    $UPPER_CONFIG_EGRESS
    $ENABLE_SELF_SERVICE
    $AUTHORIZATION_ENDPOINT
    $TOKEN_ENDPOINT
    $REVOCATION_ENDPOINT
    $END_SESSION_ENDPOINT
  ' < "/tmp/$(basename $file).tmpl" > "$file"
done
