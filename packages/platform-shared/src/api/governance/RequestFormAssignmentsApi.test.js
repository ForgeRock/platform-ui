...
describe('RequestFormAssignments Api', () => {
  ...
  it('getFormAssignmentByLcmTypeAndOperation should call api with the correct query', async () => {
    const lcmType = 'type1';
    const operation = 'operation1';

    await RequestFormAssignmentsApi.getFormAssignmentByLcmTypeAndOperation(lcmType, operation);
    expect(getMock).toBeCalledWith(`${formAssignmentUrl}?_queryFilter=objectId eq "lcm/${lcmType}/${operation}"`);
  });

  it('getFormLcmType should call api with the correct query', async () => {
    const formId = '123';
    const lcmType = 'type1';
