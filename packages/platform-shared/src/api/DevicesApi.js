 ...
 * @param {string} realm - Realm for the request
 * @param {string} userSearchAttribute - User id for the request
 * @param {string} authType - Authentication type for the request
 * @returns {Promise<object>} AM Device item
 */
export function getAuthenticationDevices(realm, userSearchAttribute, authType) {
  return generateAmApi({
    apiVersion: 'protocol=2.1,resource=1.0',
    path: `realms/root/realms/${realm}`,
  }).get(
    `/users/${userSearchAttribute}/devices/2fa/${authType}?_queryId=*`,
    { withCredentials: true },
  );
}

/**
 * Delete Authentication Device
 *
 * @param {string} realm - Realm for the request
 * @param {string} userSearchAttribute - User id for the request
 * @param {string} authType - Authentication type for the request
 * @param {string} id - id of the authentication device
 * @returns {Promise<object>} AM Device item
 */
export function deleteAuthenticationDevice(realm, userSearchAttribute, authType, id) {
  return generateAmApi({
    apiVersion: 'protocol=2.1,resource=1.0',
    path: `realms/root/realms/${realm}`,
  }, {}, false).delete(
    `/users/${userSearchAttribute}/devices/2fa/${authType}/${id}`,
    { withCredentials: true },
  );
}

/**
 * Update Authentication Device
 *
 * @param {string} realm - Realm for the request
 * @param {string} userSearchAttribute - User id for the request
 * @param {string} authType - Authentication type for the request
 ...
 * @returns {Promise<object>} AM Device item
 */
export function updateAuthenticationDevice(realm, userSearchAttribute, authType, id, payload) {
  return generateAmApi({
    apiVersion: 'protocol=2.1,resource=1.0',
    path: `realms/root/realms/${realm}`,
  }, {}, false).put(
