...
/**
 * Determines if a given field is disabled.
 * @param {Array} schemaField - The schema field of each given property.
 * @param {boolean} readOnly - A boolean value that indicates whether the form is read-only.
 * @param {object} privilegeData - The user privileges for the schema types.
 * @returns {object} - Whether the schema field is disabled or not
 *
 * @see FormEditor
 * @see FormGenerator
 */
function isDisabled(schemaField, readOnly, privilegeData) {
  const fieldType = schemaField.model?.split('.')?.[1] ?? schemaField.model;
  return (privilegeData && !includes(get(privilegeData, 'UPDATE.properties', null), fieldType)) || schemaField.readOnly || readOnly;
}
...
export function transformSchemaToFormGenerator(schema, readOnly = false, includeDefaults = false, privilegeData = null) {
  return schema.map((row) => row.fields.map((schemaField) => {
    ...
    const field = {
      ...schemaField,
      disabled: isDisabled(schemaField, readOnly, privilegeData),
