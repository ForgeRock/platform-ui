...
describe('Theme Injector Component', () => {
  ...
  describe('enduser stylesheet', () => {
    ...
    it('applies accountCardInputSelectColor as background for multiselect tags', () => {
    ...
    it('applies accountCardInputTextColor to text-dark and tag-icon inside multiselect tags', () => {
      const accountCardInputTextColor = '#1a1a2e';
      const wrapper = shallowMount(ThemeInjector, {
        props: {
          isEnduser: true,
          theme: { accountCardInputTextColor },
        },
      });
      const html = wrapper.html();
      expect(html).toContain('.multiselect__tag .text-dark');
      expect(html).toContain('.multiselect__tag .multiselect__tag-icon::after');
      expect(html).toContain(accountCardInputTextColor);
    });

    it('defaults multiselect tag text to #23282e when accountCardInputTextColor is unset', () => {
      const wrapper = shallowMount(ThemeInjector, {
        props: { isEnduser: true, theme: {} },
      });
      const html = wrapper.html();
      expect(html).toContain('.multiselect__tag .text-dark');
      expect(html).toContain('#23282e');
    });
  });
