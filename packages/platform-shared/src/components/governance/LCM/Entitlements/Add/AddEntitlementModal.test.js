...
describe('AddEntitlementModal', () => {
  ...
  it('does not include justification in common when custom form is active', async () => {
    ...
    // The Entitlements/Add component references userStore in the template but does not
    // import it in the script (pre-existing bug). Provide it via global.mocks so the
    // v-if="form && !userStore.adminUser" expression does not throw when form is truthy.
    setupTestPinia({ user: {} });
    routerPush = mockRouter().routerPush;
    wrapper = mount(AddEntitlementModal, {
      global: {
        plugins: [i18n],
        stubs: ['GovResourceSelect', 'FrFormBuilder'],
        mocks: {
          $router: { push: jest.fn() },
          userStore: { adminUser: false },
        },
      },
