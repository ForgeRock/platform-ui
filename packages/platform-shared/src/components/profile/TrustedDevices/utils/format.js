...
export function formatDevices(result) {
  ...
  return Promise.all(sortDevicesByDate(result).map(async (deviceData, index) => {
    const {
      alias, identifier, lastSelectedDate, location,
    } = deviceData;

    const locationData = !Number.isNaN(parseFloat(location?.latitude)) && !Number.isNaN(parseFloat(location?.longitude)) ? await parseLocationData(location) : { formattedAddress: '', locality: '', map: '' };
