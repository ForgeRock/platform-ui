...
describe('Glossary', () => {
  const setup = () => mount(CertificationTaskListGroupBy, {
    i18n,
    propsData: {
  ...
  beforeEach(() => {
    ...
    jest.spyOn(CertificationApi, 'certifyCertificationTasks').mockReturnValue(Promise.resolve());
  ...
  describe('@actions', () => {
    ...
    it('should open comments modal on accounts table', async () => {
      ...
      await wrapper.vm.$nextTick();
      const rootWrapper = createWrapper(wrapper.vm.$root);
      expect(rootWrapper.emitted()['bv::show::modal'][0][0]).toEqual('CertificationTaskCommentsAccountModal');
    });
    it('should open comments modal on entitlements table', async () => {
      ...
      await wrapper.vm.$nextTick();
      const rootWrapper = createWrapper(wrapper.vm.$root);
      expect(rootWrapper.emitted()['bv::show::modal'][0][0]).toEqual('CertificationTaskCommentsEntitlementModal');
    });
    it('should open reviewers modal on accounts table', async () => {
      ...
      await wrapper.vm.$nextTick();
      const rootWrapper = createWrapper(wrapper.vm.$root);
      expect(rootWrapper.emitted()['bv::show::modal'][0][0]).toEqual('CertificationTaskReviewersAccountModal');
    });
    it('should open reviewers modal on entitlements table', async () => {
      ...
      await wrapper.vm.$nextTick();
      const rootWrapper = createWrapper(wrapper.vm.$root);
      expect(rootWrapper.emitted()['bv::show::modal'][0][0]).toEqual('CertificationTaskReviewersEntitlementModal');
    });
    it('should have select all option on accounts table', async () => {
      const wrapper = setup();
      await flushPromises();
      findByTestId(wrapper, 'cert-select-all-tasks-accounts').trigger('click');
      await flushPromises();
      findByTestId(wrapper, 'cert-bulk-certify-accounts').trigger('click');
      await flushPromises();
      expect(wrapper.emitted()['update-details']).toBeTruthy();
      expect(wrapper.emitted()['change-saving']).toBeTruthy();
    });
    it('should have select all option on entitlements table', async () => {
      const wrapper = setup();
      await flushPromises();
      findByTestId(wrapper, 'cert-select-all-tasks-entitlements').trigger('click');
      await wrapper.vm.$nextTick();
      findByTestId(wrapper, 'cert-bulk-forward-entitlements').trigger('click');
      const rootWrapper = createWrapper(wrapper.vm.$root);
      expect(rootWrapper.emitted()['bv::show::modal'][0][0]).toEqual('CertificationTaskForwardEntitlementModal');
    });
  });
  describe('@units', () => {
