...
describe('RequestsTab', () => {
  ...
  describe('@Component Tests', () => {
    ...
    it('should list requests correctly', async () => {
      ...
      expect(findByRole(domWrapper, 'menuitem', 'Forward').exists()).toBe(true);
      await toggleActionsMenu(cellsFirstRow[2]);
    ...
    it('should show error if requests cannot be fetched', async () => {
      ...
      AccessRequestApi.getRequests = jest.fn().mockReturnValue(Promise.reject(error));

      const { wrapper } = setup();
      await flushPromises();

      expect(showErrorMessage).toHaveBeenCalledWith(error, 'There was an error retrieving the requests');
      expect(wrapper.vm.isLoading).toBe(false);
    ...
    it('should show request type ids if request types cannot be fetched', async () => {
      const error = new Error('Error fetching requests');
      AccessRequestApi.getRequestType = jest.fn().mockReturnValue(Promise.reject(error));
