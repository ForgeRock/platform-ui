...
describe(
  'Tests for Journey Choice collector',
  { tags: ['@forgeops', '@cloud'] },
  () => {
    const loginRealm = Cypress.env('IS_FRAAS') ? '/alpha' : '/';
    const journey = `QA-Choice_Collectors_${
      Cypress.env('IS_FRAAS') ? 'Cloud' : 'ForgeOps'
    }.json`;
    const locationUrl = `${
      Cypress.config().baseUrl
    }/am/XUI/?realm=${loginRealm}&authIndexType=service&authIndexValue=QA%20-%20Choice%20Collectors#/`;
    ...
    retryableBeforeEach(() => {
      ...
      cy.findByRole('heading', { name: 'Choices UI Journey' }).should(
        'be.visible',
      );
    });
    ...
    function loginEnduser() {
      ...
      cy.findByRole('heading', { name: 'Radio Choice Collector!' }).should(
        'be.visible',
      );
    }
    ...
    function chooseRadioOption(option) {
      // Choose 'option'
      cy.findByRole('radio', { name: option })
    ...
    it('[TC-12138] Page Node with Radio Choice Collector - default value is checked and working', () => {
      // Login as Enduser
      loginEnduser();
    ...
    it('[C20733] Page Node with Radio Choice Collector - Unconnected node returns to the start of a Journey', () => {
      ...
      cy.findByTestId('FrAlert')
        .should('exist')
        .contains("Sorry that isn't right. Try again.");
    });

    it('[C20731] Page Node with Message Node - Positive answer works correctly', () => {
      ...
      cy.findAllByRole('heading', { timeout: 20000 })
        .contains(`${userName}`)
        .should('be.visible');
    });

    it('[TC-12139] Page Node with Message Node - Negative answer works correctly', () => {
      const nodeToLoad = 'Buttons test!';
    ...
    it('[C20726] Message Node - Positive answer works correctly', () => {
    ...
    it('[TC-12140] Message Node - Negative answer works correctly', () => {
      const nodeToLoad = 'Message node!';
    ...
    it('[TC-12137] Page Node with Select Choice Collector - Unhappy path works correctly', () => {
      const nodeToLoad = 'Select Choice Collector!';
