...
Given('Admin/User navigates to {string} page', (page) => {
  ...
  switch (page) {
    case 'admin login':
      ...
      break;
    case 'end user login':
      cy.visit(`${Cypress.config().baseUrl}/am/XUI/?realm=${realm}&authIndexType=service&authIndexValue=Login`);
      break;
    case 'registration':
      cy.visit(`${Cypress.config().baseUrl}/am/XUI/?realm=${realm}&authIndexType=service&authIndexValue=Registration`);
      break;
    default:
